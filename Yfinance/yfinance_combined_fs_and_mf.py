...
# Display the results
print("\n--- Top 5 Ranked by EBIT / EV ---")
for rank, symbol, ebit_to_ev in ranked_ebit_to_ev[:5]:
    print(f"{rank}. {symbol}: EBIT / EV = {ebit_to_ev if ebit_to_ev is not None else 'N/A'}")

print("\n--- Top 5 Ranked by ROIC ---")
for rank, symbol, roic in ranked_roic[:5]:
    print(f"{rank}. {symbol}: ROIC = {roic if roic is not None else 'N/A'}")

print("\n--- Top 5 Combined Rankings (Based on Sum of Ranks) ---")
for symbol, ebit_rank, roic_rank in combined_ranking[:5]:
